...
// --- encode-direction channel views (read-only, zero-copy) -------------------
//
// channels: either a single 2-D (n_channels, n_samples) array, or a Python sequence of 1-D
// array-likes, one per audio channel - AC-3 order (Table 5.8, LFE last, see docs/library/
// index.md's own "Channel order" convention) either way. expected_len == 0 skips the
// per-channel length check (used nowhere today, kept for the one call site that legitimately
// has no fixed length).
//
