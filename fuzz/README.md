...
| `fuzz_sendspin_messages` | `ac3::sendspin::messages`' readers - the core messages after the handshake, from `client/hello` to `group/update`, each read in the specification's dialect and aiosendspin 9.1.1's. Whatever reads is written back out, and that text must read and write back to itself |
