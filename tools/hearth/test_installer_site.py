...
class InstallerSite(unittest.TestCase):
    ...
    def run_site(self, get=None) -> tuple[int, str]:
        output = io.StringIO()
        with (
            ...
            code = installer_site.main(["--release", "latest", "--out", str(self.out)])
    ...
    def test_no_release_with_sink_firmware_yet_leaves_an_empty_installer(self) -> None:
        ...
        self.assertEqual(
            json.loads((self.out / "index.json").read_text("utf-8")), {"version": "", "images": []}
        )
