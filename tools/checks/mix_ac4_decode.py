...
g_dialog and g_assoc of 0 dB, at -6 and -10 dB, and at +9 dB, which g_dialog_max caps.
...
def expected_matrix(p, found, refs, records, dialogue_gain, associated_gain, de_gain):
    """The matrix from every channel of the substreams decoded alone, in `found`'s order, to
    each output channel of the presentation."""
    ...
    blocks[anchor['substream_index']] = rows * h
    if de_member is not None:
        blocks[de_member['substream_index']] = rows * w
    for m in found:
...
def check_stream(cli, stream, work):
    ...
    # Each substream alone, by the presentation of its group alone.
    refs = {}
    for i, p in enumerate(presentations):
        ...
        if decodable and p['presentation_config'] is None and p['presentation_id'] is not None:
            samples = decode(cli, stream, work / f"{stream.stem}-{i}.wav",
                             f"presentation-id={p['presentation_id']}")
            refs[found[0]['substream_index']] = (samples, WAV_ORDER[samples.shape[1]])
    ...
    for i, p in enumerate(presentations):
        ...
        if not decodable or p['presentation_config'] is None:
            continue
        if any(m['substream_index'] not in refs for m in found):
            failures.append(f"{stream.stem} presentation {i}: a substream has no presentation "
                            "of its own to measure it by")
            continue
        stacked = np.hstack([refs[m['substream_index']][0] for m in found])[SKIP:]
        for dialogue_gain, associated_gain in ((0.0, 0.0),) if hybrid else SETTINGS:
            de_gain = DE_GAIN_DB if hybrid else 0.0
            ...
            expected = expected_matrix(p, found, refs, trace_records(trace, 0), dialogue_gain,
                                       associated_gain, de_gain)
            name = (f"{stream.stem} presentation {i} (id {p['presentation_id']}, config "
                    f"{p['presentation_config']}) at {dialogue_gain:+g}/{associated_gain:+g} dB")
            found_failures, summary = check(name, out, stacked, expected)
            failures += found_failures
            checked += 1
            print(f"{name:<72} {summary}", flush=True)
