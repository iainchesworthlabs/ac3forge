...
def parse_substream_group_info(r, fs_index, frame_rate_factor):
    ...
    if b_channel_coded:
        ...
        for _ in range(n_lf_substreams):
            if r.bits(1):  # b_ajoc
                info = parse_substream_info_ajoc(r, fs_index, frame_rate_factor, b_substreams_present)
                kind = 'ajoc'
            else:
                info = parse_substream_info_obj(r, fs_index, frame_rate_factor, b_substreams_present)
                kind = 'obj'
            if b_hsf_ext:
                parse_hsf_ext_substream_info(r, b_substreams_present)
...
def main():
    ...
    for i, sub in enumerate(substreams):
        kind = 'audio (chan/ajoc/obj)' if sub['is_audio'] else 'other (presentation/EMDF-payloads/OAMD)'
