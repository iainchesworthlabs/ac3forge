 ...
 */
export function* iterateBoxes(buffer, rangeStart = 0, rangeEnd = buffer.length) {
...
function* findChildren(buffer, box, type) {
    for (const child of iterateBoxes(buffer, box.start, box.end)) {
        if (child.type === type)
            yield child;
    }
...
function readTkhdTrackId(buffer, tkhd) {
    const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
...
function readMdhdTimescale(buffer, mdhd) {
    const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
...
function readHdlrType(buffer, hdlr) {
...
function readTfdtBaseMediaDecodeTime(buffer, tfdt) {
    const view = new DataView(buffer.buffer, buffer.byteOffset, buffer.byteLength);
 ...
 */
export function extractFragments(buffer) {
    const fragments = [];
