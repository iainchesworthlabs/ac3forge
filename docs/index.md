...
| `ac4::ac4` | An AC-4 (ETSI TS 103 190-1/-2) sync-frame/TOC/presentation/channel-coded-substream-group bitstream inspector, the same probe/JSON contract extended to a second codec (`ac3cli probe` auto-detects it). Parse-and-inspect, not decode: audio content is reported by byte range, never decoded, and A-JOC/object/OAMD substream groups are refused cleanly rather than misparsed. Links nothing from `ac3::forge` and knows nothing about AC-3 — a peer codec, not an extension. See [Validation](verification.md#ac-4). |
