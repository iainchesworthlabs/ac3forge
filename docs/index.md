...
| `iamf::iamf` | A standalone writer for AOM's IAMF (Immersive Audio Model and Formats) v1.1.0 — codec-blind, same shape as `mp4::mp4`/`matroska::matroska`. Writes a channel-based Audio Element (7.1.4ch, `ipcm` substreams) plus a Mix Presentation, in IAMF's own ISO-BMFF encapsulation. E-AC-3 can never be an IAMF codec, so this is a decode → rewrap bridge — see [IAMF writing](library/iamf.md) and `examples/mux_iamf.cpp` for the `Eac3Decoder` round trip. Object elements and a reader are not yet implemented. |
