...
from E-AC-3's `mixmdate`, or from Annex D's `xbsi1` for a `bsid`-6 AC-3 stream that sends one
(see below) — without acting on it: `target` is always what the caller asked for. A caller that
