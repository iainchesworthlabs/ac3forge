...
| `delta_allocation` | `true` | §7.2.2.6 delta bit allocation, as for AC-3: the corrections chosen per run and the second fit that weighs them. `false` skips both - the first level of the encoders' effort axis, measured on the ESP32-S3 page - and the stream has `dbaflde` clear; the tools string spells it `nodelta`. |
