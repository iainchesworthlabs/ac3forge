...
| `search` | `kNone` | Roadmap EQ13: per-frame search over `dbpbcod` against `ac3::quality`'s decoded-domain distortion, instead of the fixed `dbpbcod` 3 EQ3 measured its way to on average. CBR only (`FrameConfig::vbr` unset) - silently inert under VBR/ABR, the same documented boundary EQ5 draws around AHT streams, not a rejected configuration. `kDistortion` only: `kPerceptual` is accepted but inert too, on the same grounds [Decision search](encoding-ac3.md#decision-search) already found it for AC-3. Candidates are `{kAllocCodes' 3, Table E1.4's 2}` - one axis, not the two AC-3's search has, because E-AC-3 has no per-frame `fgaincod` to search yet (`frmfgaincode` stays 0 unconditionally). Measured on real CC0 stereo material at 96-640 kbit/s: effect negligible everywhere tried (see ROADMAP.md's EQ13 and EQ8 entries) - `dbpbcod` alone, without `fgaincod` moving alongside it, has very little left to find once EQ3's own sweep already put `dbpbcod` at its best average value. CLI: `search=distortion`/`search=perceptual`/`search=off`. |
