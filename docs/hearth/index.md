...
synchronised network audio. It lists AC-4 files and shows what they contain, but does not play
them.

!!! note "Status as of 2026-09-26: the desktop player plays to network sinks; AC-4 is not played"
    **The desktop player `ac3hearth` plays to local outputs and to network sinks.** It runs on
    ...
    that Music Assistant uses; Music Assistant itself has not been tested.
...
| Windows, Linux and macOS | `ac3hearth` (the desktop window, Qt 6.8+) with its engine and Network page; `ac3hearth-testsink` and `ac3hearth-testserver` development tools | Engine and Sendspin interoperability tests run in CI, and the window builds there on all three platforms |
...
- **Play AC-4.** The queue accepts `.ac4` files and the Media page shows their table of contents:
  the presentations and substream groups. Playback is not available. The app has no AC-4 decoder
  yet, and no local output takes AC-4 as a bitstream. Its Sendspin extension role has a data type
  for AC-4 bursts, which only the development test sink takes; no ESP32 sink decodes AC-4. The
  [AC-4 decoder in the library](../library/ac4.md) is not connected to the app.
