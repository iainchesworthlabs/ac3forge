...
Both are machine-wide security settings. Turning them off to run a demo driver is a trade worth
weighing: it is what a development machine does, and more than you should ask of a machine you
depend on. This goes away when the driver is attestation-signed: it will then install with the
application and need neither setting.
...
Crucible still runs. Taps, placements and every output mode work; you will hear the direct
