...
from a run against `main` at `be71f454` on 2026-09-09, `arm-none-eabi` GCC 14.2.1 under QEMU
10.2.1's `mps2-an385`; `build-footprint` in `.github/workflows/_build.yml` reproduces them on
every push, and `tools/checks/run_baremetal_probe.sh` reproduces them locally.
...
| | Bytes |
|---|---|
| `.text` (code + read-only data) | 223,260 |
| `.data` (initialised) | 400 |
| `.bss` (zero-initialised) | 97,280 |
| **Image total** | **320,940** (313.4 KiB) |
...
| Object | `.text` | `.bss` |
|---|---|---|
| `probe.cpp.obj` (the harness itself — fixtures, checks, allocator hooks) | 55.5 KiB | 48.8 KiB |
| `eac3_decoder.cpp.obj` (all of Annex E) | 38.4 KiB | 0 |
| `eac3_tools.cpp.obj` (spx/ecpl band geometry + §3.5.5 reconstruction) | 20.3 KiB | 10.3 KiB |
| `decoder.cpp.obj` (AC-3) | 18.5 KiB | 0 |
| `mdct.cpp.obj` (inverse transform, fast path only) | 15.9 KiB | 14.6 KiB |
| `joc.cpp.obj` (§6 object reconstruction from the bed) | 13.5 KiB | 0 |
| `oamd.cpp.obj` (§H.1 object metadata) | 6.8 KiB | 0 |
| `qmf.cpp.obj` (DC10's QMF-domain JOC reconstruction) | 6.0 KiB | 4.2 KiB |
| `output.cpp.obj` (`OutputStage::apply`/`mix_levels`, both decoders') | 4.9 KiB | 16 B |
...
| everything else, summed | 21.4 KiB | 769 B |
