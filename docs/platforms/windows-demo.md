...
| **S2 bitstream** | Does `PassthroughSink` in E-AC-3 and AC-3 exclusive mode lock on a real Atmos receiver from this workstation? This is DR9's Windows row. | **Confirmed 2026-09-14**, via `ac3cli` rather than the Crucible app itself — see [Windows](windows.md#audio-backend-wasapi): an Onkyo TX-RZ740 locked Dolby Digital, then Dolby Digital Plus, then Atmos/DD+ for a signed object stream (decoded to 5.0.4, audible object motion), at zero-to-near-zero underruns each run. Crucible's own output stage wraps the same `PassthroughSink`, but the app itself has not yet been run against a receiver directly |
