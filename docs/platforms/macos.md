...
whose `AudioDeviceIOProcID` reads exactly the way a real input device's does. Because
`CATapDescription` is an Objective-C class with no C entry point, this is the one place in the
project with a second compiled language: `src/audio/src/backend/macos/process_tap.mm` is the only
`.mm` in the tree, behind the plain-C++ header `process_tap.hpp` that the rest of the backend
includes, and `src/audio/CMakeLists.txt`'s `APPLE` block enables `OBJCXX` for that one file.
...
DR9 still records that no real Mac has ever run any part of this backend.

**So what is claimed is that it compiles.** The version gate is exercised for real on every macOS
CI run (`tests/backend/macos/test_macos_support.cpp`), and the backend contract case in
...
neither of which touches a tap, a consent prompt or a device. Nothing past that has been observed
to work.
...
`tests/audio/test_audio_backend.cpp` is what will exercise starting, restarting and stopping a
watcher on the CI runners — a device-free path, since registration needs no endpoint. As with the
tap, that has not been watched happening yet, and no event has ever been seen to arrive.
