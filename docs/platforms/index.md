...
| **An ESP32-S3** (ESP-IDF, FreeRTOS) | [ESP32-S3](esp32.md) | `ac3::forge_minimal` as a reusable ESP-IDF component, decode-only or encode-only | AC-3 and E-AC-3 decode **correctly**, Atmos objects included, every channel level exact against a baked-in fixture, inside internal SRAM with no PSRAM. Two [example players](esp32.md#examples) drive I2S, one of them streaming from flash, SD or HTTP. **Real time on the board**: a 5.1 E-AC-3 frame decodes in 11.0 ms of its 32 at 240 MHz, measured on an ESP32-S3-DevKitC-1 — see [Timing](esp32.md#timing); CI runs under QEMU, which cannot answer it. |
| **A board with no operating system at all** | [Bare metal](bare-metal.md) | `ac3::forge_minimal`: one static library, decode-only or encode-only, no exceptions, no RTTI | Cross-compiled `arm-none-eabi` and run on QEMU's `mps2-an385` (Cortex-M3, no FPU). A probe decodes six frames each of eleven fixtures and gates on exact per-channel levels, image size, heap peak, retained bytes and allocation counts. Correctness under emulation; no real silicon. |
