...
| **An ESP32-S3** (ESP-IDF, FreeRTOS) | [ESP32-S3](esp32.md) | `ac3::forge_minimal` as a reusable ESP-IDF component, decode-only or encode-only | AC-3 and E-AC-3 decode **correctly** — every layout to 7.1.4, every coding tool, the output stage's folds, Atmos objects reconstructed **and placed onto 7.1.4** by their positions — every level exact against baked-in fixtures, inside internal SRAM with no PSRAM; AC-3 and E-AC-3 encode 2/0 and 5.1 byte-exact with the host. Two [example players](esp32.md#examples) drive I2S, one of them streaming from flash, SD or HTTP. **Real time on the board** for every decode fixture it has run: a 5.1 E-AC-3 frame in 11.0 ms of its 32 and 7.1.4 in 28.8, at 240 MHz on an ESP32-S3-DevKitC-1 — see [Timing](esp32.md#timing); the encoders and the newest rows await the next board run, and CI runs under QEMU, which cannot answer it. [What the part can and cannot do](esp32.md#what-the-part-can-and-cannot-do) is the full table. |
