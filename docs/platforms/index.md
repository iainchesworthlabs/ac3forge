...
| **An ESP32-C3** (ESP-IDF, FreeRTOS) | [ESP32 variants](esp32.md#other-esp32-variants) | The same `ac3::forge_minimal` component - its manifest lists `esp32c3` beside `esp32s3` - decoding in the fixed-point tier, since the part has no floating-point unit at all | **Decode is correct, and that is all this row claims.** `apps/baremetal/platform/esp32c3/` is a probe target CI runs under `qemu-riscv32`: twelve of the fourteen fixtures decode, and every one of them produces PCM **identical to the x86 host's and the Cortex-M3 leg's** - three architectures, three compilers, one pinned set of hashes. The two 7.1.4 rows do not fit in 400 KB of SRAM: they need 238,094 and 244,502 bytes of heap where the part reports 249,180 free in a heap whose largest block is 114,688. **Speed on a C3 is unmeasured** - QEMU is not cycle-accurate and no board has run this. Encode is not validated here at all: both encoders are floating-point, which on this part means software floating point. |
