...
| E6 | Message 1's payload names the PSK: `psk_id` = base64url(SHA-256("sendspin-psk-id-v1" ‖ PSK)) and `psk_category` `lt`, `pr` or `sn` | M | Implements | Selects the PSK by it, after decrypting message 1 without a PSK; also accepts 9.1.1's payload without `psk_category` ([C6](#music-assistant-and-aiosendspin-911)) |
...
| C6 | Noise message 1's payload | `psk_id` and `psk_category` | `psk_id` only | Server: sends both, and a 9.1.1 client ignores the field it does not know. Player: accepts a message 1 with no `psk_category` and takes the category of the stored PSK whose `psk_id` matches; the identifier is a hash of one PSK, so it names one record |
...
| Q3 | Whether aiosendspin's fragment IDs 2 and 3, `sid` without `round`, missing Sentinel fallback and message 1 without `psk_category` will move to the specification's text, and when; and whether a peer can be told apart on the wire meanwhile | aiosendspin 9.1.1 against `pairing.md`, `messaging.md` and `connection.md` | [C1 to C6](#music-assistant-and-aiosendspin-911) |
