...
class AC3FORGE_EXPORT FrameEncoder {
   ...
   private:
    ...
    // coupling scratch, the per-frame plan, the DRC controllers, all of it -
    // lives behind this one pimpl, following the same pattern as
    // ac3::io::WavStreamReader/Writer and ac3::FrameEncoder. Impl is defined
    // in eac3_frame.cpp, so a dllexport class instantiating every implicit
    // special member is why the destructor and moves above are declared (not
    // defaulted inline) here: move-assignment's implicit reset() needs Impl
    // complete.
    struct Impl;
