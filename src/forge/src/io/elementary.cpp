    ...
    while (offset < stream.size()) {
        ...
        if (bsid <= kAc3MaxBsid) {
            ...
            if (out.access_units.empty()) {
                ...
                Acmod acmod = Acmod::k2_0;
                bool lfe = false;
                if (const auto ok = read_ac3_bsi(r, frame_bsid, bsmod, acmod, lfe); !ok) {
                ...
                out.bsid = frame_bsid;
                out.bsmod = bsmod;
                out.bit_rate_code = info->bit_rate_code;
                locations = bed_locations(acmod, lfe);
            }
            ++substreams;
            offset += info->bytes;
            continue;
        ...
        if (sub->strmtyp != static_cast<int>(eac3::StreamType::kDependent)) {
