...
// is. `rate` is the access unit's sample rate: every substream of every
// programme codes the same 1536 samples of the same frame period, so a
// substream that disagrees would desynchronise the whole unit, not just its
// own programme.
std::expected<std::vector<FrameConfig>, FrameError> programme_configs(
    const ProgrammeConfig& programme, int id, SampleRate rate) {
    ...
    for (std::size_t i = 0; i < programme.dependents.size(); ++i) {
        ...
        // Every substream codes the same 1536 samples of one programme, so a
        // dependent cannot disagree with its parent about the sample rate.
        if (dep.sample_rate != rate) {
            return std::unexpected(FrameError::kInvalidSubstream);
        }
...
std::expected<std::vector<std::vector<FrameConfig>>, FrameError> access_unit_configs(
    ...
    auto first = programme_configs({config.independent, config.dependents}, 0, rate);
    if (!first) {
            ...
            for (int n = 0; n < kSamplesPerBlock; ++n) {
                programme.tail[ch][static_cast<std::size_t>(n)] =
                    static_cast<double>(own[ch][static_cast<std::size_t>(
                        kSamplesPerFrame - kSamplesPerBlock + n)]);
            }
        }
