            ...
            for (int blk = 0; blk < kBlocksPerFrame; ++blk) {
                ...
                to_fixed25_block(std::span<const double>{source}.subspan(
                                 ...
                                 std::span{out}.subspan(static_cast<std::size_t>(plan.start),
                                                        span));
                // §7.2.2.6's real-coefficient peak (see the delta gate
                // below): a separate pass over `source` rather than fused
                // into the block conversion above, so that shared kernel's
                // contract - fixed-point conversion, nothing else - stays
                // exactly what PF5 measured.
                for (int bin = plan.start; bin < plan.endmant; ++bin) {
                    const auto ubin = static_cast<std::size_t>(bin);
                    peak_mag[ubin] = std::max(peak_mag[ubin], std::abs(source[ubin]));
                }
                extract_exponents(
