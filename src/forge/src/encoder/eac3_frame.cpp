...
std::expected<std::vector<std::byte>, FrameError> FrameEncoder::encode_frame(
    ...
    // Coupling and spectral extension answer that from geometry decided
    // above; chbwcod, when neither is in use, now answers it from the
    // frame's own spectrum, so it cannot be resolved until step 2 has run
    // the transform. Declared here because the stream lambdas below close
    // over it - nothing calls them before step 2 either, and the assignment
    // right after the transform is the only write.
    int fbw_endmant = 0;
    ...
    // Coded bandwidth (§E3.3.3, deferred from step 1 - see fbw_endmant).
    //
    ...
    // 192 kbit/s with the AHT-only tool set `auto` chooses there:
    //
    ...
    // survive are coded well enough to reach the decoder at all.
