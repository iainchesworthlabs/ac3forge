...
// Where the tracks of a channel element go in the SIMPLE and ASPX codec
// modes, ETSI TS 103 190-1 V1.4.1 clause 5.3.4: the single channel element's
// one track is C (5.3.3.1); a pair's stereo_data() makes L and R (5.3.4.1);
// the 3.0 element's L, R and C (5.3.4.2); the 5.X element's by Table 180 and
// the 7.X element's by Table 182, with Table 183's two last steps; an LFE's
// mono_data(1) is the LFE, and the tables number the tracks after it
// (src/ac4dec/ERRATA.md, "The LFE's track is not numbered in Tables 180 and
// 182").
//
...
// One of Table 183's steps: (first, second) = P (first, second), in window
// order, P from the chparam_info() at `chparam`.
struct PairStep {
...
struct ElementRoute {
    ...
    // parameters; at identity they change nothing and are left out.
    std::vector<PairStep> steps;
};

// The route of `element`, read under `ctx`, in the SIMPLE or ASPX codec mode.
// Fails when the element's parts do not add up to what its coding_config
// names: that many tracks, chparam_info()s, stereo flags and chel_matsel
// values.
[[nodiscard]] ParseResult route_element(const SubstreamContext& ctx, const ChannelElement& element,
                                        ElementRoute& out);

// The A-SPX data of an element in ASPX mode, by Part 1 Table 213: each
// aspx_data_1ch() or aspx_data_2ch() and the channels it carries, in syntax
// order. `index` counts within aspx_1ch or aspx_2ch.
struct AspxUnit {
    bool pair = false;
    int index = 0;
    std::array<Speaker, 2> speakers{};
};
[[nodiscard]] std::vector<AspxUnit> aspx_units(int ch_mode);

// Table 212: the channels companding_control() lists, in its order. Empty
// where the element's ASPX mode sends none (the 7.X element's).
[[nodiscard]] std::vector<Speaker> companded_speakers(int ch_mode);
