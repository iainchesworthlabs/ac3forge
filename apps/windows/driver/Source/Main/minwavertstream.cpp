...
--*/
{
    ...
    // The null sink: nothing is generated for a capture pin (there is none)
    // and nothing rendered is kept. What the audio engine writes into the
    // buffer is read past and discarded by UpdatePosition below.
...
)
{
    ...
    else
    {
        ...
        // A real device would DMA these bytes out now; the null sink drops
        // them on the floor. The position still advances, so the engine
        // sees a device consuming at exactly the nominal rate.
    }
...
--*/
{
    // No capture pin in the null sink: nothing to write.
...
--*/
{
    // The null sink keeps nothing it is given.
    UNREFERENCED_PARAMETER(ByteDisplacement);
}
...
--*/
{
    ...
    //
    // Nothing is ever written to disk here, so there is nothing to disable
    // for copy-protected content; the rights are honoured trivially.
    //
