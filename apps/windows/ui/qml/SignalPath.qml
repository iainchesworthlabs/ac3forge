...
//
//   applications play to  →  Desktop Atmos  →  you hear it on
...
Item {
    ...
    readonly property bool appsOnSilent: DeskController.defaultIsNullSink
    readonly property bool silentPresent: DeskController.nullSinkPresent
    ...
    GridLayout {
        ...
        Station {
            ...
            title: DeskController.defaultOutputName.length ? DeskController.defaultOutputName : qsTr("no default output")
            ...
            Text {
                ...
                text: qsTr("The silent device: nothing is heard from here. This app taps each application as it plays into it.")
                      + (DeskController.previousDefaultName.length ? " " + qsTr("%1 is restored on quit.").arg(DeskController.previousDefaultName) : "")
                color: Theme.textMuted
            ...
            DeskButton {
                ...
                text: root.appsOnSilent
                    ? qsTr("Restore %1").arg(DeskController.previousDefaultName.length ? DeskController.previousDefaultName : qsTr("the previous output"))
                    : qsTr("Send applications to %1").arg(DeskController.nullSinkName)
                enabled: root.appsOnSilent ? DeskController.previousDefaultName.length > 0 : root.silentPresent
                primary: !root.appsOnSilent && root.silentPresent
                onClicked: root.appsOnSilent ? DeskController.restoreDefault() : DeskController.moveDefaultToNullSink()
            }
        ...
        Station {
            ...
            title: DeskController.soundingCount + qsTr(" with sound · ") + DeskController.placedCount + qsTr(" placed")
            warn: !DeskController.running
            Text {
                Layout.fillWidth: true
                text: (DeskController.objectsEnabled ? qsTr("Placed applications are objects; the rest mix to the 5.1 bed.") : qsTr("No signing key: everything mixes to the 5.1 bed, placement pans within it."))
                color: Theme.textMuted
                font.pixelSize: Theme.fontSmall
                wrapMode: Text.WordWrap
            }
            Warning {
                visible: !DeskController.running
        ...
        Station {
            ...
            title: DeskController.endpointName.length ? DeskController.endpointName : qsTr("no endpoint")
            warn: DeskController.modeKey === "none"
            Text { Layout.fillWidth: true; text: DeskController.modeName; color: Theme.text; font.pixelSize: 13; elide: Text.ElideRight }
