...
std::optional<QcResult> measure_qc_eac3_rendered(std::span<const std::byte> stream) {
    ...
    if (!have_first) {
        std::println(stderr, "error: no decodable access units");
        return std::nullopt;
    }
...
bool report_qc_programme(const QcProgrammeResult& p, const std::optional<std::string>& preset_arg) {
    ...
    const auto check_one = [&](ac3::meta::QcPresetId id) {
        ...
        std::println("  {}:  [{}]", name, preset.source);
        ...
        const std::string loudness_limit =
            preset.loudness_limit == ac3::meta::QcLoudnessLimit::kCeiling
                ? std::format("limit  <= {:+.1f} LKFS", preset.target_lkfs)
                : std::format("target {:+.1f} +/-{:.1f} LKFS", preset.target_lkfs,
                              preset.tolerance_lu);
        if (p.integrated_lkfs) {
            std::println("    loudness   {}   measured {:+.2f} LKFS   delta {:+.2f} LU   {}",
                         loudness_limit, *p.integrated_lkfs, *verdict.loudness_delta_lu,
                         verdict.loudness_pass ? "PASS" : "FAIL");
        } else {
            std::println("    loudness   {}   measured n/a   FAIL", loudness_limit);
        }
        if (p.true_peak_dbtp) {
...
int run_qc(std::string_view in_path, const std::optional<std::string>& preset_arg,
    ...
    std::println("  layout={}  ({})", result->rendered ? "rendered" : "bed",
    ...
    if (result->bed_hid_dependents) {
        std::println("  note: this stream carries dependent substreams whose channels "
                     "(height, wide, rear)");
        std::println("        are NOT in the figures above - layout=rendered measures them "
                     "as well");
    }
    bool all_pass = true;
