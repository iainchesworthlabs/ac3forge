...
int run_record(std::string_view out_path, std::uint32_t seconds, std::uint32_t bitrate,
 ...
 // layout=/codec= (legacy item IO9). Before this, 'record' was stereo AC-3 and
 ...
 // Streamed to its container as it is produced (legacy item IO9), through the
...
// sink-following play's transcode-to-passthrough leg: DC9's transcode produces an
...
int run_play(std::string_view in_path, int device_index, const Options& meta) {
 ...
 // sink-following play: what the chosen sink actually accepts. The default
