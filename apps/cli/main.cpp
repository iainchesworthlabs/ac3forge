...
void print_usage() {
    ...
    std::println("+ one PES-wrapped audio PID), with PCR stamped on the audio PID every access");
    std::println("unit. Two broadcast profiles identify it, and a stream satisfies one or the");
    std::println("other: dvb (the default) writes stream_type 0x06 plus the AC3_descriptor or");
    std::println("enhanced_AC-3_descriptor ETSI EN 300 468 Annex D defines; atsc writes");
    std::println("stream_type 0x81 (AC-3) or 0x87 (E-AC-3) plus A/52 Annex A's own");
    std::println("AC-3_audio_stream_descriptor (0x81) or Annex G's E-AC-3_audio_descriptor");
    std::println("(0xCC). Either way the descriptor's identification fields come off the");
    std::println("bitstream itself — service type, channel mode, surround mode, bsid, the");
    std::println("substreams in use — with mainid=/asvc= for the service associations no");
    std::println("single elementary stream can know.");
    std::println("");
    std::println("strip-objects removes the JOC/OAMD object layer from a Dolby Digital Plus");
    std::println("stream without decoding it: the EMDF container and the addbsi object marker");
    std::println("come out, frmsiz and crc2 are re-derived, and every exponent and mantissa is");
    std::println("copied bit for bit — so the result is a plain DD+ 5.1 stream whose bed audio");
    std::println("decodes sample-identically. That is the rendition Apple's HLS authoring");
    std::println("requirements want beside an Atmos one; 'fmp4 ... fallback-51' writes both.");
    std::println("");
