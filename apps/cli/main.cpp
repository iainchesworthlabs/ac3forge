...
constexpr std::array<Command, 44> kCommands{{
    ...
    {"truehd-encode", 3, "<in.wav> <out.mlp>", "lossless MLP; 16/24-bit integer PCM in",
     topic::kNone, Needs::kNothing,
     [](const Args& x) { return run_truehd_encode(x.str(1), x.str(2)); }},
    {"truehd-decode", 3, "<in.mlp> <out.wav>", "bit-exact PCM out", topic::kNone,
     Needs::kNothing,
     [](const Args& x) { return run_truehd_decode(x.str(1), x.str(2)); }},
    {"truehd-atmos", 3, "<in.wav> <out.mlp> [objects] [paths.txt]",
     "every source channel a dynamic object as its own lossless channel; optional motion from "
     "a scene file (same formats as atmos-path)",
