...
void print_usage() {
    ...
    std::println("       eac3-encode, atmos-encode, decode and probe only. e.g.:");
    ...
    std::println("");
    std::println("probe reports what a stream DECLARES, without decoding its audio: bsid, rate");
    std::println("       (fscod2 half rates included), acmod/lfeon and the resolved layout,");
    std::println("       bsmod, chanmap, the substream map, frame and access-unit counts,");
    std::println("       duration, measured bit rate and VBR spread, dialnorm/compr/dynrng");
    std::println("       ranges, EMDF payload ids, OAMD/JOC with complexity_index and the");
    std::println("       object/bed configuration, whether an authenticity tag is present,");
    std::println("       per-frame CRC validity and how often each coding tool was used.");
    std::println("       json=1 emits the ac3forge.probe/1 document instead (docs/cli/");
    std::println("       commands.md documents it as a stable contract); detail=frames adds");
    std::println("       a per-access-unit dump and detail=blocks adds each block's Annex E");
    std::println("       tools and exponent strategies. Exit code is non-zero if any frame");
    std::println("       failed its CRC or the parser refused it, so this works as a gate.");
    std::println("");
