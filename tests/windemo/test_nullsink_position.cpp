...
TEST_CASE("nullsink clock: nothing moves until Run", "[windemo][nullsink]") {
...
TEST_CASE("nullsink clock: the position advances at exactly the nominal rate", "[windemo][nullsink]") {
    PositionClock clock = configured();
...
TEST_CASE("nullsink clock: the position never runs backwards", "[windemo][nullsink]") {
    PositionClock clock = configured();
    clock.run(1000);
    u64 last = 0;
...
TEST_CASE("nullsink clock: pause freezes, run resumes from where it left off", "[windemo][nullsink]") {
    PositionClock clock = configured();
    clock.run(0);
    clock.pause(100 * kMs);
...
TEST_CASE("nullsink clock: stop returns to zero", "[windemo][nullsink]") {
    PositionClock clock = configured();
    clock.run(0);
...
TEST_CASE("nullsink clock: packets complete as the position passes their ends", "[windemo][nullsink]") {
    PositionClock clock = configured();
    clock.run(0);
...
TEST_CASE("nullsink clock: a late timer owes every packet it missed, once each", "[windemo][nullsink]") {
...
TEST_CASE("nullsink clock: completions after a pause line up with the position", "[windemo][nullsink]") {
    PositionClock clock = configured();
    clock.run(0);
...
TEST_CASE("nullsink clock: an unconfigured packet size owes nothing", "[windemo][nullsink]") {
    PositionClock clock;
