...
using ac3::windemo::TapPool;
using ac3::windemo::testing::FakeDevices;
...
TEST_CASE("tap pool opens one tap per application at the pool's width", "[windemo][tap_pool]") {
...
TEST_CASE("tap pool closes taps whose application left and keeps the rest", "[windemo][tap_pool]") {
    auto devices = std::make_shared<FakeDevices>();
...
TEST_CASE("tap pool reports the applications whose tap refused to open", "[windemo][tap_pool]") {
    auto devices = std::make_shared<FakeDevices>();
    TapPool pool(devices, 2);
...
TEST_CASE("tap pool reads the same frame count from every tap, as signal", "[windemo][tap_pool]") {
    auto devices = std::make_shared<FakeDevices>();
    TapPool pool(devices, 2);
