...
using ac3::windemo::PlacementSmoother;
using ac3::windemo::PlacementTarget;
using Catch::Approx;

TEST_CASE("a positioned slot glides to its target and settles exactly", "[windemo]") {
...
TEST_CASE("snap jumps a slot to its target with no lag", "[windemo]") {
    PlacementSmoother smoother(3.0);
    std::vector<ac3::oba::ObjectPlacement> out(kObjectSlots);
...
TEST_CASE("freeing a slot fades the gain out in place", "[windemo]") {
    PlacementSmoother smoother(3.0);
    std::vector<ac3::oba::ObjectPlacement> out(kObjectSlots);
...
TEST_CASE("the bed slots are always the pinned speaker placements", "[windemo]") {
    PlacementSmoother smoother(3.0);
    std::vector<ac3::oba::ObjectPlacement> out(kObjectSlots);
    smoother.step(out);
...
TEST_CASE("an idle positioned slot is silent at the centre", "[windemo]") {
    PlacementSmoother smoother(3.0);
    std::vector<ac3::oba::ObjectPlacement> out(kObjectSlots);
    smoother.step(out);
...
TEST_CASE("a zero time constant is immediate", "[windemo]") {
