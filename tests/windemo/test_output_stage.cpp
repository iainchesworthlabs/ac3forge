...
using namespace ac3::windemo;
using namespace ac3::windemo::testing;
...
TEST_CASE("output stage: an E-AC-3 capable endpoint gets a burst sink and E-AC-3 bursts", "[windemo][output_stage]") {
...
TEST_CASE("output stage: with a key the same endpoint carries Atmos", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
    devices->devices = {realtek_default(), null_sink(), hdmi_avr()};
    OutputStage stage(config_over(devices));
...
TEST_CASE("output stage: an AC-3-only endpoint gets the DD 5.1 leg", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
    auto avr = hdmi_avr();
...
TEST_CASE("output stage: a multichannel PCM endpoint gets decoded 5.1", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
...
TEST_CASE("output stage: a stereo endpoint gets a Lo/Ro decode, and the bypass a bed fold", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
...
TEST_CASE("output stage: headphones render the engine's objects through the bypass", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
...
TEST_CASE("output stage: a probe that changes the answer switches sinks mid-stream", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
    devices->devices = {null_sink(), realtek_default()};
    OutputStage stage(config_over(devices));
    CHECK(stage.reprobe(false).mode == OutputMode::kStereo);
...
TEST_CASE("output stage: a sink that refuses to start leaves no output and a reason", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
...
TEST_CASE("output stage: a full sink is waited on briefly, then counted as an underrun", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
    devices->devices = {null_sink(), hdmi_avr()};
...
TEST_CASE("output stage: stop tears the sink down and submit is then a no-op", "[windemo][output_stage]") {
    auto devices = std::make_shared<FakeDevices>();
    devices->devices = {null_sink(), realtek_default()};
    OutputStage stage(config_over(devices));
    stage.reprobe(false);
