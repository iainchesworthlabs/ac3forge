...
| Applications (`ac3cli`, Hearth, Forge GUI, bindings) | **In progress** — every `ac3cli` command reads or writes AC-4 where it can: `transcode` both ways, `record`/`live` with `codec=ac4`, `monitor`, `play`, `qc`, `levels`, `loudness`, `probe`, `spdif`, and `fmp4` as a CMAF track (I1); Hearth, the Forge GUI and the bindings follow in I2 to I6 |
