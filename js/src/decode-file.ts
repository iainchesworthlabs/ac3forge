...
function accumulate(
  ...
  bytes: Uint8Array,
  accessUnits: readonly { offset: number; length: number }[],
): DecodedProgram {
  const channelChunks: Float32Array[][] = [];
  const foldChunks: Float32Array[][] = [];
  ...
  const objectAudioChunks: Float32Array[][] = [];
  ...
  for (const unit of accessUnits) {
    ...
    if (channelLabels.length === 0) channelLabels = result.channelLabels;
    ...
    if (result.objectCount > 0) {
      ...
      for (let i = 0; i < Math.min(objectCount, result.objectCount); i++) {
        ...
        if (frame) objectPositions[i]!.push(...frame.position);
        const audio = decoder.objectAudio(i);
        if (audio) objectAudioChunks[i]!.push(Float32Array.from(audio));
      }
  ...
  for (const entry of flushed) {
    if (!entry.holdBack && entry.channelCount === channelChunks.length) {
      appendChannels(channelChunks, entry.channelCount, entry.frameSamples, (ch) =>
        decoder.flushedChannel(entry.flushIndex, ch),
      );
    }
  }
  ...
  const objectStartSeconds =
    objectStartFrameIndex <= 0 || objectFrameSize === 0
      ? 0
...
function appendChannels(
  ...
  count: number,
  frameSamples: number,
  read: (channel: number) => Float32Array | null,
): void {
...
function rmsBlocks(samples: Float32Array, blockSize: number): Float32Array {
  if (blockSize <= 0 || samples.length === 0) return new Float32Array(0);
